...
class TestCheckResponse:
    ...
    async def test_ollama_exception_returns_error(self, monkeypatch):
        """Ollama fails → ERROR. No auto-fallback: the caller (or the
        owner via /checker) decides which driver runs; the checker itself
        does one attempt and returns."""
        async def fake_ollama(prompt, model, base_url="http://localhost:11434", timeout=30.0):
    ...
    async def test_provider_exception_returns_error(self, monkeypatch):
        """provider driver, provider fails → ERROR. Symmetric to the
        ollama-failure case: single attempt, no fallback."""
