...
syne init
    ...
    │   ├─ Google Gemini (recommended — free via OAuth)
    │   ├─ Detect existing OpenClaw/Gemini CLI credentials?
    │   │   ├─ YES → Reuse OAuth tokens
    │   │   └─ NO → Open browser for Google OAuth PKCE flow
    │   └─ Or: OpenAI / Together (paid, API key)
    │
    ├─ Step 2: Telegram bot token (from @BotFather)
    │
    ├─ Step 3: Start PostgreSQL + pgvector Docker container
    │
    ├─ Step 4: Initialize database schema (12 tables + seed data)
    │
    ├─ Step 5: Set agent name & motto
    │
...
Minimal `.env` file (generated by `syne init`):
...
That's it. **Only 2 environment variables.** All credentials (Telegram token, OAuth tokens, API keys) are stored in PostgreSQL, configured via `syne init` or through conversation. This keeps secrets in one secure place instead of scattered across files.
