...
-- v0.14.2: codex max_tokens null → 32768 (API needs explicit limit for reasoning models)
DO $$
DECLARE
    _models jsonb;
    _updated jsonb := '[]'::jsonb;
    _model jsonb;
    _changed boolean := false;
BEGIN
    SELECT value::jsonb INTO _models FROM config WHERE key = 'provider.models';
    IF _models IS NULL THEN RETURN; END IF;

    FOR _model IN SELECT * FROM jsonb_array_elements(_models)
    LOOP
        IF _model->>'driver' = 'codex'
           AND _model->'params' IS NOT NULL AND _model->'params' != 'null'::jsonb
           AND (_model->'params'->'max_tokens' = 'null'::jsonb
                OR NOT (_model->'params' ? 'max_tokens'))
        THEN
            _model := jsonb_set(_model, '{params,max_tokens}', '32768'::jsonb);
            _changed := true;
        END IF;
        _updated := _updated || jsonb_build_array(_model);
    END LOOP;

    IF _changed THEN
        UPDATE config SET value = _updated, updated_at = NOW() WHERE key = 'provider.models';
    END IF;
END $$;
