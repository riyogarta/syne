    ...
    LOOP
        ...
        IF model->'params' IS NOT NULL AND model->'params' != 'null'::jsonb THEN
            ...
            defaults := CASE driver
                ...
                WHEN 'codex'         THEN '{"temperature": 0.7, "max_tokens": 8192,  "thinking_budget": 4096,  "top_p": 1.0,  "top_k": null, "frequency_penalty": 0,    "presence_penalty": 0}'::jsonb
...
-- v0.12.5: tune codex defaults (max_tokens 16384→8192, thinking_budget 8192→4096)
DO $$
DECLARE
    models jsonb;
    updated jsonb := '[]'::jsonb;
...
BEGIN
    ...
    LOOP
        IF model->>'driver' = 'codex' THEN
            p := model->'params';
            IF p IS NOT NULL AND p != 'null'::jsonb THEN
                IF (p->'max_tokens') = '16384'::jsonb THEN
                    model := jsonb_set(model, '{params,max_tokens}', '8192'::jsonb);
                    changed := true;
                END IF;
                IF (p->'thinking_budget') = '8192'::jsonb THEN
